...
def upgrade_graham_py():
    ...
    text = replace_once(
        ...
        "return real-data fields",
    )

    GRAHAM.write_text(text, encoding="utf-8")
    print("graham.py upgraded")


def upgrade_app_py():
    text = APP.read_text(encoding="utf-8")
    if '("11", "Margin of Safety"' in text and '("12", "Growth Formula"' in text:
        print("app.py already shows 12 Graham criteria")
        return
    ...
    text = replace_once(
        ...
        'st.caption("Twelve-point Graham scorecard using Yahoo Finance reported fundamentals plus Moody\'s Aaa corporate bond yield from FRED for the growth formula. Missing observations remain N/A.")',
        "Graham page caption",
    )
    ...
    text = replace_once(
        ...
        'm4.metric("Unavailable", f"{12 - assessed}/12")',
        "12-point denominator",
    )
    ...
    text = replace_once(
        ...
        '''    st.info("Sales, debt and working capital are displayed in ₹ crore (₹1 crore = ₹10,000,000); EPS, CMP and BVPS are per share in rupees. The current calendar year is excluded from the 20-year dividend test.")\n    growth_note = "N/A" if pd.isna(d.get("growth_rate_pct", np.nan)) else f"{d['growth_rate_pct']:.2f}%"\n    yield_note = "N/A" if pd.isna(d.get("aaa_yield", np.nan)) else f"{d['aaa_yield']:.2f}%"\n    eps_note = "N/A" if pd.isna(d.get("growth_eps", np.nan)) else f"₹{d['growth_eps']:.2f}"\n    yield_date = d.get("aaa_yield_date") or "N/A"\n    st.caption(f"Growth Formula inputs: latest reported EPS = {eps_note}; g = {growth_note} historical EPS growth proxy from the 10-year first/last 3-year averages; Y = {yield_note} Moody's Seasoned Aaa Corporate Bond Yield (FRED DAAA, {yield_date}).")\n''',
        "growth formula data note",
    )

    APP.write_text(text, encoding="utf-8")
    print("app.py upgraded")


if __name__ == "__main__":
    upgrade_graham_py()
    upgrade_app_py()
    print("Graham scorecard ready: 12 points")
